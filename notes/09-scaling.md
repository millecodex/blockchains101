...
# Lecture 09: Scaling
## Contents
- [Lecture 09: Scaling](#lecture-09-scaling)
  - [Contents](#contents)
  - [Blockchain Stack](#blockchain-stack)
  - [The Trilemma](#the-trilemma)
  - [Scalability](#scalability)
    - [Time-to-Finality (TTF) and State Bloat](#time-to-finality-ttf-and-state-bloat)
  - [On-Chain Scaling (Layer 1)](#on-chain-scaling-layer-1)
    - [Segregated Witness](#segregated-witness)
    - [Sharding](#sharding)
  - [State Channels (Layer 2s; not on the base chain)](#state-channels-layer-2s-not-on-the-base-chain)
    - [Lightning Network](#lightning-network)
  - [Side Chains](#side-chains)
  - [Rollups](#rollups)
  - [Modular Blockchain Stack](#modular-blockchain-stack)
- [Summary](#summary)
...
The Trilemma posits that a blockchain system can only simultaneously optimize two out of the following three properties (as also discussed in [Lecture 08](08-decentral.md)):
* **Decentralization:** Distributing control and participation broadly so that no single entity can censor transactions, alter history, or control the network.
* **Security:** Defending against attacks (like 51% attacks) and ensuring that transactions, once confirmed, are immutable and final.
* **Scalability:** Handling a high volume of transactions rapidly and cheaply, without imposing prohibitive hardware requirements on nodes.

This lecture focuses on scalability, however, it is intrinsically linked to the other two properties. To simply increase scalability by raising hardware requirements fundamentally harms decentralization.

## Scalability
Two design decisions in Bitcoin fundamentally limit the number of transactions the network can process: the block-time and the block-size, hard-coded to ten minutes and 1 MB. Each block in bitcoin is 1 MB, or 1,000,000 bytes and the average transaction size is 580 bytes so $\frac{1000000}{500}=2000$ standard transactions per block. The average block time is 600 seconds[^1] and so 
$\text{throughput }=\frac{2000}{600}=3.33\text{ transactions per second.}$

[^1]: 600s is the theoretical blocktime, in practice blocks have been mined on average every 594 seconds in the past two years.

In a global network there must be time for a 1  MB block to flood the network, allowing for consensus, before the subsequent block is found. These two properties dictate a trade-off and in Bitcoin's case limit the capacity to around 3 transactions per second. For a global payments system this is orders of magnitude smaller than it needs to be[^2]. The table shows difference scenarios for block size and tx size. Segwit has increased the effective block size by approximately 1MB. (More below.)

[^2]: As of 2026 The VISA network averages 83,000 TPS [coinlaw.io](https://coinlaw.io/visa-statistics/); comparatively PayPal averaged 900 TPS in 2025 with peak volumes up to 3000.

| Scenario               | Block Size                          | Avg. Tx Size        | Calculation               | Result        |
| :--------------------- | :---------------------------------- | :------------------ | :------------------------ | :------------ |
| **Legacy (Small Tx)**  | $1,000,000 \text{ bytes}$           | $250 \text{ bytes}$ | $(1,000,000 / 250) / 600$ | **6.67 TPS**  |
| **Legacy (Typical)**   | $1,000,000 \text{ bytes}$           | $500 \text{ bytes}$ | $(1,000,000 / 500) / 600$ | **3.33 TPS**  |
| **SegWit (Optimized)** | $\approx 2,000,000 \text{ bytes}^*$ | $250 \text{ bytes}$ | $(2,000,000 / 250) / 600$ | **13.33 TPS** |
...
### Time-to-Finality (TTF) and State Bloat
While TPS is the most commonly cited metric, it is not the only dimension of scalability. **Time-to-Finality (TTF)** measures how quickly a user can be certain that their transaction is irreversible. A high-TPS network is less useful if transactions remain probabilistic for hours. Different scaling solutions make different trade-offs between throughput and finality. Another critical consequence of scaling is **State Bloat**. As previously discussed, processing high TPS causes the ledger to grow, which places economic and hardware stress on node operators. Solutions like state expiry, pruning, and statelessness remain active areas of research.

TPS is one factor in the scaling debate and many solutions have been proposed such as Schnorr signatures, sharding, Merkelized Abstract Syntax Trees, and Segwit (next). These improvements are considered may require the software code base to be upgraded and possibly forked and sit at layer 2 in the stack; not altering consensus but improving the application layer. 


## On-Chain Scaling (Layer 1)
### Segregated Witness
This backwards compatible software upgrade separates witness data from the list of transaction inputs. A bitcoin transaction has a witness, or digital signature, attached to it that must be verified for it to be spent in the future. Segregating this part makes room for more transactions in a block without affecting the transactions themselves because the inputs and outputs are preserved and the signatures (witness) are kept in another tree. Antonopoulos lists several advantages of Segwit including network and storage scaling (Antonopoulos, 2017). Segwit transactions introduced a weighting limit rather than a size limit which means more can fit into a block. The result is an approximate doubling in block size from 1 MB to 2 MB and throughput from $\approx$3 to $\approx$7 TPS.
...
> <img width="3438" height="2496" alt="image" src="https://github.com/user-attachments/assets/eb65e24b-9512-4263-a8e5-f4ce5a616495" />
> Figure: A sharded database splits the table horizontally to allow for smaller index size. The issue becomes communication, for example when searching for record A you need to search both shards. Source: Digital Ocean https://www.digitalocean.com/community/tutorials/understanding-database-sharding.
 
Blockchains can also be sharded. Every node in a standard blockchain stores a copy of the entire ledger and validates all activity. Partitioning the ledger into parent/child shards, or even horizontally, can improve scalability. Given enough nodes to validate a portion of the ledger it can remain reasonably secure. Each shard will manage its own history and only be updated if required. The downside is that decentralisation erodes with more shards added to the network. Current blockchain sharded projects include Near's Nightshade, Zilliqa, Elastico, OmniLedger. Ethereum planned to scale via sharding but has abandoned it in favour of rollups (below).

## State Channels (Layer 2s; not on the base chain)
...
### Lightning Network
The lightning network represents an off-chain scaling solution for Bitcoin because the functionality occurs in a separate network with only the endpoints connecting to the Bitcoin blockchain. All the transacting happens between lightning nodes that don't need to write to the blockchain.
...
**The Centralized Sequencer Trade-off:** In modern rollup architectures, the entity responsible for collecting, ordering, and batching transactions is called the **Sequencer**. While rollups inherit the strict security guarantees of the L1, almost all major rollups currently operate with a *centralized* sequencer (run by the core development team). This introduces a single point of failure and potential censorship risk. This is a classic Trilemma compromise: sacrificing some decentralization to achieve massive scalability, with plans to decentralize the sequencer role in the future.
...
|       | Storage               | State Channel           | Side Chains                                  | Rollups                                             | Data Availability (DA) / Modular                |
| ----- | --------------------- | ----------------------- | -------------------------------------------- | --------------------------------------------------- | ----------------------------------------------- |
| tech. | sharding; compression | DIY off-chain; multisig | Child/Parent; two-way peg; detached security | Optimistic; ZK (zero-knowledge); inherited security | DAS; blob transactions; off-chain DA publishing |
| ex:   | segwit (btc)          | Lightning (btc)         | Liquid (btc)                                 | Arbitrum/Optimism/Base (eth) [Optimistic]           | EIP-4844 blobs / Proto-Danksharding (eth)       |
| ex:   | taproot (btc)         | ~~Raiden~~ (eth)        | Polygon PoS (eth)                            | zkSync/StarkNet/Linea (eth) [ZK]                    | Celestia (standalone)                           |
| ex:   |                       |                         | ~~Plasma~~ (eth)                             |                                                     | EigenDA / Avail (eth)                           |
...
* Some more keywords: state expiry, pruning, and statelessness

# Further Reading - the very short list
* What is the Blockchain Trilemms [Binance Academy](https://www.binance.com/en/academy/articles/what-is-the-blockchain-trilemma)
* What are Ethereum Layer 2 Blockchains? [Coinbase](https://www.coinbase.com/learn/crypto-basics/what-are-ethereum-layer-2-blockchains-and-how-do-they-work)
* The Lightning Network Explained [Investopedia](https://www.investopedia.com/terms/l/lightning-network.asp)
* What is a Modular Blockchain? [Celestia](https://celestia.org/learn/basics-of-modular-blockchains/modular-and-monolithic-blockchains/)
* Danksharding on Ethereum [Ethereum](https://ethereum.org/roadmap/danksharding/)
